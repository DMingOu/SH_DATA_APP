...
public class BottomDrawerLayout extends LinearLayout {
    ...
    public BottomDrawerLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.BottomDrawerLayout);
        if (typedArray != null) {
            defaultHeight = typedArray.getDimension(R.styleable.BottomDrawerLayout_defaultHeight, 0);
            realHeight = typedArray.getDimension(R.styleable.BottomDrawerLayout_realHeight, 0);
            typedArray.recycle();
        }
        init();
    }
...
//        }
    ...
    //设置默认露出的高度，参数单位px
    public BottomDrawerLayout setDefaultHeight(float defaultHeight) {
        this.defaultHeight = defaultHeight;
        return this;
    }

    //设置预计露出的高度，参数单位px
    public BottomDrawerLayout setRealHeight(float realHeight) {
...
//                }
    ...
    //titleview可以添加多个，必须是在activity布局内声明的view，且不在BottomDrawerLayout内部，titleview将自己消费touch事件
    public void addTitleView(View... views) {
        this.titleViews = views;
    }

    //touchView只能添加一个，必须是在activity布局内声明的view，且必须在BottomDrawerLayout内部，touchview内部的子view将自己消费touch事件
    public BottomDrawerLayout addTouchView(View touchView) {
        this.touchView = touchView;
        return this;
    }

    public BottomDrawerLayout setOnScrollStateListener(OnScrollStateListener onScrollStateListener) {
